...
def options():
    ...
    modes_group_.add_argument('--reset',
                        dest='reset',
                        action='store_true',
                        help='Reset ip/mac addresses and ip routes of phy, upstream and bridge to original ones and recreate and new bridges allowing supplicant to reauthenticate with the authenticator')
...
# ------------------------------------------------------------------
    ...
    missing_reset_params = any([
        args.phy is None,
        args.upstream is None,
    ])
    if args.reset and missing_reset_params:
        parser.error("--reset requires the --phy and --upstream flags")
    ...
    if args.rogue_gateway and missing_nec_params:
        parser.error("--rogue-gateway requires --client-mac, --switch-mac, --phy")
